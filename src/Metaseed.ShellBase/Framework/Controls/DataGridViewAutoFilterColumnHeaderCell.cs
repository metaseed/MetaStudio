...
namespace Slb.RCS.ComplianceTest
{
    /// <summary>
    ...
    public class DataGridViewAutoFilterColumnHeaderCell : DataGridViewColumnHeaderCell
    {
        ...
        /// </summary>
        private string[] _checkedFilterValues;
        ...
        public void ShowDropDownList()
        {
            ...
            // update checked list from last checkedItems
            foreach (var s in _checkedFilterValues)
            {
                for (var i =0; i< dropDownListBox.Items.Count; i++)
                {
                    if (dropDownListBox.Items[i].ToString().Equals(s))
                    {
        ...
        private void SetDropDownListBoxBounds()
        {
            ...
            // ListBox borders.
            var dropDownListBoxHeight = 2;
            var currentWidth = 0;
            var dropDownListBoxWidth = 0;
            ...
            using (var graphics = dropDownListBox.CreateGraphics())
            {
                foreach (String filter in filters.Keys)
                {
                    var stringSizeF = graphics.MeasureString(
            ...
            var clientLeft = 1;
            var clientRight = DataGridView.ClientRectangle.Right;
        ...
        protected Int32 DropDownListBoxMaxHeightInternal
        {
            get
            {
                ...
                var dataGridViewMaxHeight = DataGridView.Height -
                ...
                var listMaxHeight = dropDownListBoxMaxLinesValue * dropDownListBox.ItemHeight + 2;
        ...
        {
            dropDownListBox.ItemCheck += new ItemCheckEventHandler(DropDownListBox_Check);
        ...
        private void DropDownListBox_Check(object sender, ItemCheckEventArgs e)
        {
            Debug.Assert(DataGridView != null, "DataGridView is null");
            if (e.NewValue == CheckState.Checked)
                if(e.Index == 0) // uncheck others if All checked
                {
                    for(var i = 0; i < dropDownListBox.CheckedItems.Count; i++)
                    {
                        var item = dropDownListBox.CheckedItems[i];
                        var ind = dropDownListBox.Items.IndexOf(item);
                        dropDownListBox.BeginInvoke((MethodInvoker)(()=>dropDownListBox.SetItemChecked(ind,false)));
                    }

                }
                else // uncheck All if others checked
                {
                    dropDownListBox.SetItemChecked(0, false);
                }
        ...
        private void PopulateFilters()
        {
            ...
            var list = new ArrayList(data.Count);
            ...
            foreach (var item in data)
            {
                ...
                if (ictd != null)
                {
                    var properties = ictd.GetProperties();
        ...
        private void UpdateFilter()
        {
            ...
            if (dropDownListBox.CheckedItems.Count == _checkedFilterValues.Length)
            {
                var equal = true;
                foreach (var item in dropDownListBox.CheckedItems)
                {
                    if(!_checkedFilterValues.Contains(item))
                    {
                        equal = false; break;
                    }
                }
            ...
            _checkedFilterValues = dropDownListBox.CheckedItems.Cast<object>().Select(it => it.ToString()).ToArray();

            // Cast the data source to an IBindingListView.
            var data = 
                DataGridView.DataSource as IBindingListView;
            ...
            if (_checkedFilterValues.Any(it => it.Equals(All)))
            {
            ...
            string GetFilter(string selected)
            {
                string filter;
                switch (selected)
                {
            ...
            newColumnFilter = string.Join(" OR ", _checkedFilterValues.Select(GetFilter));
            ...
            if (String.IsNullOrEmpty(newFilter) || string.IsNullOrEmpty(newColumnFilter))
            {
        ...
        private class FilterListBox : CheckedListBox
        {
            /// <summary>
